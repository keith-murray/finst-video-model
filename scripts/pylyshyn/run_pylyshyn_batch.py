"""
Batch runner for the pylyshyn arm's "1/10 cued" experiment
(claude/2026_08_14/TODO.md's "Let's run some simple experiments" section):
n_cued fixed at 1 (of the fixed n_objects=10), 20 seeds, each seed run twice
-- once with probe_on_target=True, once False -- so the two runs share an
identical underlying scene (placement/cueing/motion all derive from `seed`
alone in pylyshyn.physics.build_objects; only which object gets probed
...
OpenRouter's docs). Measured cost during design: ~$0.002-0.005/trial.
...
Usage:
    ...
    uv run python scripts/pylyshyn/run_pylyshyn_batch.py --resume <batch_id>
"""
...
RESULT_FIELDS = [
    "trial_id", "seed", "n_cued", "probe_on_target", "probe_is_target",
...
def run_one_trial(batch_dir: str, seed: int, probe_on_target: bool) -> dict:
    ...
    result = {
        "trial_id": cfg.trial_id, "seed": seed, "n_cued": N_CUED,
...
def main():
    ...
    if not os.path.isfile(config_path):
        with open(config_path, "w") as f:
            json.dump({
                "batch_id": batch_id, "arm": "pylyshyn", "model": MODEL,
                "provider": PROVIDER, "reasoning": REASONING, "n_cued": N_CUED,
                "seeds": args.seeds,
            }, f, indent=2)
