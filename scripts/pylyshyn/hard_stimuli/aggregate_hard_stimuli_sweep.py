"""
Aggregates `run_hard_stimuli_sweep.py`'s results.csv into a per-model
accuracy/d'/cost summary, per `claude/2026_09/2026_09_16/TODO.md`'s Part 1.
Independently reverifies the nearest-neighbor heuristic's accuracy on the
underlying 100-trial set (expected ~50% by construction, per
`generate_hard_stimuli.py`) via the production
`nearest_neighbor_heuristic.heuristic_accuracy_by_group` function, not just
trusted from the generation report.

Writes results/pylyshyn/hard_stimuli/hard_all_models_sweep/summary.json.

Usage:
    uv run python scripts/pylyshyn/hard_stimuli/aggregate_hard_stimuli_sweep.py
"""
...
RESULTS_CSV = "results/pylyshyn/hard_stimuli/hard_all_models_sweep/results.csv"
TRIALS_ROOT = "data/pylyshyn/hard_all_models_sweep/trials"
OUT_PATH = "results/pylyshyn/hard_stimuli/hard_all_models_sweep/summary.json"
