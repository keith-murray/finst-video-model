"""
Approximates the dollar cost of `run_hard_stimuli_sweep.py`'s full 100-trial
x 5-model sweep before running it, per `claude/2026_09/2026_09_16/TODO.md`'s
"let's also try to approximate the cost of this sweep as best as possible."

No dedicated cost-estimator exists yet in this repo -- the established
per-session convention (checked against several past SUMMARY.md writeups,
e.g. 2026-08-19/2026-09-01) is: live-query OpenRouter's per-model pricing
(per [[feedback-verify-pricing]], don't trust stale docs) as a sanity
check, then run a small number of real pilot trials with
`return_usage=True` to get actual measured $/trial (OpenRouter's own
`usage.cost` field, which folds in whatever multiplier/host-specific rate
actually applied -- more reliable than a manual token-count x price-per-
token calculation), then extrapolate pilot $/trial x 100 per model.

Pilot trials reuse PILOT_N_TRIALS of the real stimulus set (already
generated by `generate_hard_stimuli.py`) and the real stretched video +
no-reasoning config from `run_hard_stimuli_sweep.py`, so their measured
cost is representative of the real sweep's per-trial cost (same video
size/duration, same question, same model/provider config) -- these three
calls per model also land in the real sweep's results.csv since they use
the same trial_ids, so no run budget is wasted.

Usage:
    uv run python scripts/pylyshyn/hard_stimuli/estimate_hard_sweep_cost.py
    uv run python scripts/pylyshyn/hard_stimuli/estimate_hard_sweep_cost.py --pilot-n 5
"""
...
from run_hard_stimuli_sweep import (
...
OUT_PATH = "results/pylyshyn/hard_stimuli/hard_all_models_sweep/cost_estimate.json"
...
def run_pilot(model: str, trial_ids: list[str], already_ran: set) -> dict:
    ...
    results.csv (same schema/fieldnames as run_hard_stimuli_sweep.py) so its
