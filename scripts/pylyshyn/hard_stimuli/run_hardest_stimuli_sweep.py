...
`generate_hardest_stimuli.py` through several OpenRouter models, reasoning
off -- `claude/2026_09/2026_09_16/TODO.md`'s Part 4: "rerun the sweep...
drop qwen3.5 for google/gemini-3.8-flash and drop glm-5v-turbo for
qwen/qwen3.8-27b."

`google/gemini-3.8-flash` is excluded from this no-reasoning pass: live-
checked via OpenRouter's `/models` endpoint (2026-09-16) and its reasoning
is mandatory (`{"mandatory": true, "supported_efforts": ["high","medium",
"low"]}`, no way to disable) -- confirmed with the user, who chose to drop
it from the no-reasoning condition entirely rather than approximate "off"
with a low-effort run or substitute a different model. It's added back for
the reasoning="medium" condition (`run_hardest_stimuli_thinking_sweep.py`).
`qwen/qwen3.8-27b` replaces `z-ai/glm-5v-turbo`; confirmed non-mandatory
reasoning (`{"mandatory": false, "supported_efforts": ["xhigh","medium",
"low"]}`, matches [[reference-local-qwen-reasoning-effort]]'s no-"high"
finding for this model family).

Model set here (4): gemma-4-31b-it, qwen3.6-plus, kimi-k3, qwen3.8-27b.
Same provider pins carried over from `run_hard_stimuli_sweep.py` (kimi-k3
pinned to moonshotai; gemma excludes the `together` endpoint that 400s on
non-serverless access).

Writes to results/pylyshyn/hard_stimuli/hardest_stimuli_sweep/
{results.csv,config.json} -- resumable.

Usage:
    uv run python scripts/pylyshyn/hard_stimuli/run_hardest_stimuli_sweep.py
"""
...
MODEL_REASONING = {
    ...
    "qwen/qwen3.8-27b": {"enabled": False},
}
MODEL_PROVIDER = {
...
RESULTS_DIR = "results/pylyshyn/hard_stimuli/hardest_stimuli_sweep"
...
def main():
    ...
    if not os.path.isfile(config_path):
        with open(config_path, "w") as f:
            json.dump({
                "run_name": "hardest_stimuli_sweep", "arm": "pylyshyn",
