"""
Runs the 100-trial, heuristic-neutralized pylyshyn n_objects=3 set from
`generate_hard_stimuli.py` through several OpenRouter models --
`claude/2026_09/2026_09_16/TODO.md`'s Part 1: "return to openrouter models
and test them on 100 videos of the Pylyshyn task ... with three objects
(and only one cue) ... with stretched videos and no reasoning."

Model set is the 5 non-reasoning models from
`run_pylyshyn_speed_sweep_models.py`'s MODEL_REASONING, minus
`google/gemma-4-31b-it:free` (dropped per the user: shares a noisy
rate-limited pool across all OpenRouter users, per
[[reference-openrouter-provider-routing-gotchas]], and would be flaky for a
100-trial run). `moonshotai/kimi-k3` keeps the same provider pin (avoids a
fallback endpoint that 400s on video input).

Reuses the exact stimulus set generated once by `generate_hard_stimuli.py`
(same `video_stretched.mp4` per trial, stretched-only per the user's
choice) rather than regenerating per model.

Writes to results/pylyshyn/hard_stimuli/hard_all_models_sweep/{results.csv,config.json}
-- results.csv written incrementally, re-running resumes automatically
(skips (model, trial_id) pairs already present).

Usage:
    uv run python scripts/pylyshyn/hard_stimuli/run_hard_stimuli_sweep.py
    uv run python scripts/pylyshyn/hard_stimuli/run_hard_stimuli_sweep.py --models google/gemma-4-31b-it
"""
...
MODEL_PROVIDER = {
    ...
    # Added after 8/100 gemma trials in the first pass fell back to a
    # "together" endpoint that 400s ("Unable to access non-serverless
    # model... visit https://api.together.ai/models/... to create a
    # dedicated endpoint") -- same class of routing-fallback issue as
    # kimi-k3's moonshotai pin above, see
    # [[reference-openrouter-provider-routing-gotchas]]. Excludes just the
    # bad provider (rather than pinning to one host) to keep the same
    # multi-provider diversity the rest of the sweep saw.
    "google/gemma-4-31b-it": {"only": ["deepinfra", "coreweave", "crusoe", "parasail", "modelrun"], "allow_fallbacks": False},
}
MAX_TOKENS = 8192

TRIALS_ROOT = "data/pylyshyn/hard_all_models_sweep/trials"
RESULTS_DIR = "results/pylyshyn/hard_stimuli/hard_all_models_sweep"
...
def main():
    ...
    if not os.path.isfile(config_path):
        with open(config_path, "w") as f:
            json.dump({
                "run_name": "hard_all_models_sweep", "arm": "pylyshyn",
