"""
Bar chart for `claude/2026_09/2026_09_16/TODO.md`'s Part 1: compares
accuracy across several OpenRouter models on the 100-trial, n_objects=3
pylyshyn set deliberately constructed so the nearest-neighbor position
heuristic sits at chance (`generate_hard_stimuli.py`), stretched video, no
reasoning. Follow-up to [[project-status-2026-09-14]]'s finding that
heuristic-hard trials are especially hard for gemma-4-31b-it -- this checks
whether that holds across models generally.

Error bars are SEM, bar-plus-errorbar structure mirrors
`plot_gemma_local_n4_heuristic_chance_bar.py` / every other `plot_*.py` in
this directory.

Reads `aggregate_hard_stimuli_sweep.py`'s summary.json rather than
recomputing from results.csv, so the two stay in sync.

Usage:
    uv run python scripts/pylyshyn/hard_stimuli/plot_hard_stimuli_sweep.py
"""
...
MODEL_COLORS = {
    ...
    "qwen/qwen3.5-397b-a17b": "#2a78d6",
    "moonshotai/kimi-k3": "#2fa85a",
    "z-ai/glm-5v-turbo": "#c2a02a",
}
MODEL_LABELS = {
...
SUMMARY_PATH = "results/pylyshyn/hard_stimuli/hard_all_models_sweep/summary.json"
OUT_PATH = "results/pylyshyn/hard_stimuli/hard_all_models_sweep/accuracy_by_model.png"
...
def main():
    ...
    fig.suptitle(
        ...
        "accuracy across models on the heuristic-neutralized-to-chance 100-trial set\n"
        "(error bars: SEM; dashed line: nearest-neighbor heuristic / chance)",
