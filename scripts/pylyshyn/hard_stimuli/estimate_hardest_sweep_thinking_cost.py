"""
Approximates the dollar cost of `run_hardest_stimuli_thinking_sweep.py`'s
full 100-trial x 5-model, reasoning="medium" sweep before running it --
...
passes combined -- this script covers the reasoning="medium" half; see
`estimate_hardest_sweep_cost.py` for the no-reasoning half). Adds
`google/gemini-3.8-flash` back in for this condition (its reasoning is
mandatory, so it's excluded from the no-reasoning pass entirely -- see
`run_hardest_stimuli_sweep.py`'s docstring).

Same convention as Part 2's `estimate_hard_sweep_thinking_cost.py`: live
pricing sanity check + real pilot trials with `return_usage=True` for
actual measured $/trial, since reasoning-token spend is not reliably
predictable from `max_tokens` or model size alone, per
[[reference-openrouter-reasoning-max-tokens]].

Pilot trials reuse the same 100-trial stimulus set and get appended to the
real sweep's results.csv, so no pilot spend is wasted once the full sweep
runs.

Usage:
    uv run python scripts/pylyshyn/hard_stimuli/estimate_hardest_sweep_thinking_cost.py
    uv run python scripts/pylyshyn/hard_stimuli/estimate_hardest_sweep_thinking_cost.py --pilot-n 5
"""
...
from run_hardest_stimuli_thinking_sweep import (
