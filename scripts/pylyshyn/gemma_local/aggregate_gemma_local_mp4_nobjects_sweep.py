"""
Aggregates raw cluster_response_mp4.json outputs (written by
scripts/pylyshyn/gemma_local/run_gemma_local_batch_mp4.py on the cluster, then rsynced
back down) into a scored results.csv, reusing this project's existing
True/False parsing and signal-detection scoring
(finst_video_model.scoring). Sibling of
scripts/pylyshyn/gemma_local/aggregate_gemma_local_nobjects_sweep.py -- same trial dirs
(data/pylyshyn/gemma_local_nobjects_sweep/trials/), but reads the mp4
variant's response file and writes to a separate results dir so neither run
...
and seed vary -- so those constants are hardcoded rather than swept.

Usage:
    uv run python scripts/pylyshyn/gemma_local/aggregate_gemma_local_mp4_nobjects_sweep.py
"""
...
MODEL_TAG = "google/gemma-4-31b-it-local-mp4"
...
def main():
    ...
    parser.add_argument(
        "--out-root", type=str, default="results/pylyshyn/gemma_local/gemma_local_mp4_nobjects_sweep",
    )
    ...
    with open(config_path, "w") as f:
        json.dump({
            "run_name": "gemma_local_mp4_nobjects_sweep",
            "arm": "pylyshyn",
            "model": MODEL_TAG,
