"""
Aggregates raw cluster_response_nframes{N}.json outputs (written by
...
since num_frames is a property of the inference run (recorded only in each
cluster_response_nframes{N}.json), not of TrialConfig/ground_truth.json the
way rotation_deg is -- the existing aggregator's "group by a config column"
shape doesn't fit this sweep axis.
...
RESULT_FIELDS = [
    ...
    "probe_on_target", "seed", "num_frames", "probe_is_target", "response",
    "predicted", "outcome", "prompt_tokens", "output_tokens",
    "generation_time_s", "error",
]


def load_trial_row(trial_dir: str, trial_id: str, num_frames: int, response_file: str) -> dict | None:
    gt_path = os.path.join(trial_dir, "ground_truth.json")
    response_path = os.path.join(trial_dir, response_file)
    if not os.path.isfile(gt_path):
        print(f"[{trial_id}, n_frames={num_frames}] missing ground_truth.json, skipping")
        return None
    if not os.path.isfile(response_path):
        print(f"[{trial_id}, n_frames={num_frames}] no {response_file} yet, skipping")
        return None
...
def main():
    ...
    for num_frames in args.num_frames:
        response_file = f"cluster_response_nframes{num_frames}.json"
        for trial_id in trial_ids:
            row = load_trial_row(
                os.path.join(args.trials_root, trial_id), trial_id, num_frames, response_file,
            )
            if row is not None:
                rows.append(row)

    n_expected = len(trial_ids) * len(args.num_frames)
    n_found = len(rows)
    print(f"{n_found}/{n_expected} (trial, num_frames) pairs scored")
    ...
    print(f"\n--- d' by num_frames (n={len(scorable)} scorable pairs) ---")
    by_num_frames = {}
    for row in scorable:
        by_num_frames.setdefault(row["num_frames"], []).append(row)
    for num_frames in sorted(by_num_frames):
        stats = compute_d_prime(by_num_frames[num_frames])
        print(
            f"num_frames={num_frames}: d'={stats['d_prime']:.2f} "
