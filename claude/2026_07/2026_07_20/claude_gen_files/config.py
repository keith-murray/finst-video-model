"""
Trial configuration for the VLM (video-to-text) FINST/MOT capacity experiment.

Unlike the Veo arm, we fully simulate and render the video ourselves, so we
have exact ground truth for every circle at every frame -- no reverse
engineering required. The model's job is purely to answer a text question
about which circles were cued, using letter labels we overlay only in the
final "answer frame."
"""
...
class TrialConfig:
    ...
    def total_duration_s(self) -> float:
        return self.cue_flash_s + self.tracking_s + self.label_s
